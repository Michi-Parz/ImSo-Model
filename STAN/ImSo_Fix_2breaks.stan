...
transformed data {
  ...
  // Segments: 2 breaks => 3 segements => 4 betas
  int n_para;
  int n_beta;
  int first_bp;
...
transformed parameters {
  ...
  b_points[1] = 50 + b_point_diff;
  b_points[2] = b_points[1] + b_point_diff2;
  // Expected value
  vector[21] mu;
  ...
  designMat[,3] = fmax(log_freq - log(b_points[1]), 0);
  designMat[,4] = fmax(log_freq - log(b_points[2]), 0);

  // vector[21] intercept = fe_betas[1]*rep_vector(1.0, 21);
  // vector[21] base = fe_betas[2]*log_freq;
  // 
  // vector[21] hinge1 = fe_betas[3]*fmax(log_freq - log(b_points[1]), 0);
  // vector[21] hinge2 = fe_betas[4]*fmax(log_freq - log(b_points[2]), 0);
  // 
  // mu = intercept + base + hinge1 + hinge2;
...
model {
  ...
  # Zweiter Bruchpunkt bei so 1000
  # Deshalb 1000-86=914
  b_point_diff2 ~ exponential(1/914.0);

  // AR coeff
  shift_coeff ~ normal(0,1);

  // Data covariance
  variances ~ exponential(1/225.0);
  // Data distrubution
