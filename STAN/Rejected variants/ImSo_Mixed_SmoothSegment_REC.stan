...
parameters {
  ...
  real fe_ls; // ln series
  real fe_lsi; // ln series inner...
  ...
  real<lower=0> hyper_v_ls;
  real<lower=0> hyper_v_lsi;
  // Kovarianz
...
transformed parameters {
  ...
  for (j in 1:21) {
    mu[i,j] = intercept[i] + slope1[i] * log_freq[j] + ls[i]*log(1+lsi[i]*freq[j]);
...
model {
  fe_intercept ~ normal(94, 4.5);
  fe_slope1 ~ normal(-11,1);
  fe_d_slope2 ~ normal(8.8,2);
  fe_bp1 ~ normal(111, 10);
  ls ~ normal(-49, 16);
  lsi ~normal(0.0002, 0.0001);

  hyper_v_int ~ exponential(0.01);
  hyper_v_slope1 ~ exponential(0.01);
  hyper_v_slope2 ~ exponential(0.01);
  hyper_v_bp1 ~ exponential(0.01);
  hyper_v_ls ~ exponential(0.01);
  hyper_v_lsi ~ exponential(0.01);
  ...
  re_var[5] ~ exponential(1.0/hyper_v_ls);
  re_var[6] ~ exponential(1.0/hyper_v_lsi);
